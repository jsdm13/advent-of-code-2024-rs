...
fn evaluate(sum: i64, acc: i64, expression: &[i64]) -> bool {
    ...
    let tmp_conc = (acc * 10u32.pow(expression[0].ilog10() + 1) as i64) + expression[0];

    if expression.len() == 1 {
        return tmp_add == sum || tmp_mul == sum || tmp_conc == sum;
    }

    evaluate(sum, tmp_add, &expression[1..])
        || evaluate(sum, tmp_mul, &expression[1..])
        || evaluate(sum, tmp_conc, &expression[1..])
}

fn main() {
    println!(
        "Result: {}",
        include_str!("../../day07a/input.txt")
            .lines()
            .map(|line| {
                let (left, right) = line
                    .split_once(':')
